...
resource "aws_cloudfront_distribution" "api" {
  count = var.ec2_public_dns != "" ? 1 : 0

  origin {
    domain_name = var.ec2_public_dns
    origin_id   = "EC2-API-${var.project_name}-${var.environment}"
  ...
  default_cache_behavior {
    allowed_methods        = ["DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT"]
    cached_methods         = ["GET", "HEAD"]
    target_origin_id       = "EC2-API-${var.project_name}-${var.environment}"
    compress               = true
    viewer_protocol_policy = "redirect-to-https"
  ...
  tags = {
    Name        = "${var.project_name}-${var.environment}-api-cloudfront"
    Environment = var.environment
    Project     = var.project_name
  }
}