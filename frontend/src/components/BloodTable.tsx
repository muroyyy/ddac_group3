...
const BloodTable: React.FC = () => {
  ...
  return (
    <div className="overflow-x-auto rounded-xl shadow-lg border border-brand-100 bg-white">
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-brand-600 text-white text-sm uppercase tracking-wider">
        ...
        <tbody className="divide-y divide-brand-100">
          {compatibilityData.map((row, index) => (
            <tr 
              key={row.type} 
              className={`hover:bg-brand-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
            >
              <td className="p-4 font-bold text-brand-700 border-r border-brand-50">{row.type}</td>
